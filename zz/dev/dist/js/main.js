...
window.onload = function() {
	...
	initChangePositionOnResize([{'parent':'.share-tab','child':'#share','start':'800','position':'left','holder':'.view_columns'},
	{'parent':'.download-tab','child':'#download','start':'650','position':'left','holder':'.view_columns'}]);
}
...
function sortButton() {
	...
	$(".wrapper").on('click', function (event) {
		if (!$(event.target).closest(".btnSort, .dropabble").length) {
			if ($(".drop_sorting").hasClass("open")) {
				$(".drop_sorting").removeClass("open");
			}
		}
	});
}
...
function sliderComunity() {
	$(".slider-comunity").slick({
		dots: false,
		infinite: false,
		arrows: true,
		slidesToShow: 5,
...
function btnSearch() {
	...
	$(".main").on('click', function (event) {
		if (!$(event.target).closest(".btn_search, .search").length) {
			if ($(".header").hasClass("choice")) {
				$(".header").removeClass("choice");
				// $(".bottom_header").slideUp(100);
			}
		}
	});
}
...
function lang() {
	...
	$(".wrapper").on('click', function (event) {
		if (!$(event.target).closest(".button_lang, .droplang").length) {
...
function hamburger() {
	...
	$(".wrapper").on('click', function (event) {
		if (!$(event.target).closest(".hamburger, .navigation_general").length) {
			if ($(".header").hasClass("open_navigation")) {
				$(".header").removeClass("open_navigation");
			}
		}
	});
}
...
function filter() {
	...
	$(".wrapper").on('click', function (event) {
		if (!$(event.target).closest(".sticky_filter, .aside_grid").length) {
...
function inputFocus() {
	var inputArr = getArrFromPseudo(document.querySelectorAll('input')); 
	if (inputArr.length > 0) {
		inputArr.forEach(function(item){
			item.addEventListener("focus", function(e){
				if (!this.value) {
					toggleClass(this.parentElement,"in-focus");
				}
			});
...
function initCustomSelect() {
	$('.js-multy-select').select2({maximumSelectionLength: 3});
	$('.js-select').select2();
}
...
// $(document).ready(function() {
//     $.fn.bannerResize();
//     document.querySelector(".sp_banner iframe");
//     initMasonry()
// }),
...
//     setTimeout(initMasonry, 1e3)
// }),
// function(e) {
//     e.fn.bannerResize = function() {
//         var t = e(".grid-item.banner").children("div")
//           , n = t.width()
//           , s = n / 300
//           , o = Math.round((n - n * s) / (2 * s) * -1);
//         t.find("div").removeAttr("style"),
//         t.find(".sp_wrap").each(function() {
//             e(this).css("-webkit-transform", "scale(" + s + ")").css("-moz-transform-scale(" + s + ")"),
//             e(this).css("top", .8333 * o).css("left", o)
//         })
//     }
// }(jQuery),
// $(document).ajaxComplete(function() {
